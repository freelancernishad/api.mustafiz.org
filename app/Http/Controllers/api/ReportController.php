   ...
   public function getDashboardStats()
{
    ...
    $baseCategories = ['student', 'senior', 'disable', 'orphan', 'homeless', 'refugee'];
    $categoryCounts = [];

    // Get counts for known categories
    foreach ($baseCategories as $category) {
        $categoryCounts[$category] = User::where('category', $category)->count();
    }

    // Count for 'other' category (i.e., not in the base categories)
    $categoryCounts['other'] = User::whereNotIn('category', $baseCategories)->count();
